...
- **Write policy is per field**, not global (#252): `tagsPolicy` (`reconcile` | `merge` | `preserve`), `descriptionPolicy` and `titlePolicy` (`overwrite` | `preserve`). `metadata.ts`'s `methodFor` maps each to an `updateFrontMatter` method — `reconcile`→`replace`, `merge`→`append`, `overwrite`→`update`, `preserve`→`update_if_empty`. `reconcile` is the only tags policy that can remove a tag, and it depends on the current tags being sent with the request (#251); `replace` exists because `update` is typed for a scalar and would write the list as a comma-joined string, after which Obsidian stops indexing the field (#230)
